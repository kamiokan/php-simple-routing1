 ...
 */
function path_info(string $domain_url): string
{
    if (isset($_SERVER['PATH_INFO'])) {
        return $_SERVER['PATH_INFO'];
    } else if (isset($_SERVER['REQUEST_URI'], $_SERVER['SCRIPT_NAME'])) {
        $url = parse_url($domain_url . $_SERVER['REQUEST_URI']);
 ...
 * ルーティングを行います。
 *
 * @param string $domain_url 末尾にスラッシュなしのドメインURL(ex. https://example.com)
 * @param array $map ルーティングマップの設定 ( array( array(リクエストメソッド, パスパターン, コールバック), ... )。例: array( array('GET', '/users/:id', 'var_dump') ))
 * @param string|null $method リクエストされたメソッド。指定しない場合は $_SERVER['REQUEST_METHOD'] が使用されます。
 * @param string|null $path リクエストされたパス。指定しない場合は $_SERVER['PATH_INFO'] (または $_SERVER['PATH_INFO'] 相当) が使用されます。
 * @return void
 */
function path_route(string $domain_url, array $map, string $method = null, string $path = null)
{
    $method = strtoupper(is_null($method) ? $_SERVER['REQUEST_METHOD'] : $method);
    if (is_null($path)) $path = path_info($domain_url);
...
path_route(
    ...
    [
        ...
        // POST の例です。
        ['POST', '/form/', function () {
            eh(filter_input(INPUT_POST, 'name') . ' が送信されました。');
        }],
        // パラメータ付きのURLのルーティングの例です。
        ['GET', '/users/:id', function ($params) {
            eh("ユーザー {$params['id']} さんのページです。");
        }],
        // 404 の例です。
        ['*', '404', function () {
            echo 'ページが見つかりません。';
